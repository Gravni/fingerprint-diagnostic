...
import { auditStoredBlobReferences } from "./fp-blob.mjs";
...
const GUM_RESULTS = Object.freeze([
  "granted", "denied", "timeout", "no-device", "device-start-failure", "aborted",
  "not-supported", "error", "unsupported",
]);
const GUM_DETAIL_PATHS = Object.freeze([
  "permissioned.getUserMedia.failure", "permissioned.getUserMedia.attempts",
...
const nullOrString = (entry) => entry === null || typeof entry === "string";
const plainObject = (entry) => entry && typeof entry === "object" && !Array.isArray(entry);

function validGumAttempt(attempt, index) {
  return plainObject(attempt) && attempt.attempt === index + 1
    && plainObject(attempt.constraints)
    && typeof attempt.constraints.audio === "boolean" && typeof attempt.constraints.video === "boolean"
    && (attempt.constraints.audio || attempt.constraints.video)
    && ["granted", "rejected", "timeout"].includes(attempt.outcome)
    && nullOrString(attempt.name) && nullOrString(attempt.message)
    && (attempt.outcome === "granted" ? attempt.name === null : true)
    && Number.isFinite(attempt.durationMs) && attempt.durationMs >= 0
    && typeof attempt.visibilityState === "string" && attempt.visibilityState.length > 0
    && ["live", "pagehide"].includes(attempt.lifecycle);
}

function validGumSingle(single) {
  if (single === null) return true;
  return plainObject(single) && ["granted", "rejected", "timeout"].includes(single.outcome)
    && nullOrString(single.name) && nullOrString(single.message)
    && Number.isFinite(single.durationMs) && single.durationMs >= 0
    && Array.isArray(single.trackKinds) && single.trackKinds.every((kind) => ["audio", "video"].includes(kind))
    && (single.outcome === "granted" || single.trackKinds.length === 0);
}

/** Typed getUserMedia evidence rows (Codex v4.5): shape + internal consistency. */
function validateGetUserMediaEvidence(gumResult, value, byPath) {
  const reason = (text) => ({ valid: false, reason: text, attempts: [], audioOnly: null, videoOnly: null, lifecycle: null, error: null });
  const missing = GUM_DETAIL_PATHS.filter((path) => byPath.get(path)?.status !== "ok");
  if (missing.length) return reason(`missing or non-ok: ${missing.join(",")}`);
  if (!GUM_RESULTS.includes(gumResult)) return reason("result outside the typed enum");
  const failure = value("permissioned.getUserMedia.failure");
  const attempts = value("permissioned.getUserMedia.attempts");
  const audioOnly = value("permissioned.getUserMedia.audioOnly");
  const videoOnly = value("permissioned.getUserMedia.videoOnly");
  const lifecycle = value("permissioned.getUserMedia.lifecycle");
  const secureContext = value("permissioned.getUserMedia.secureContext");
  if (failure !== (gumResult === "granted" ? "none" : gumResult)) return reason("failure does not restate result");
  if (!Array.isArray(attempts) || !attempts.every(validGumAttempt)) return reason("attempts malformed");
  if (!validGumSingle(audioOnly) || !validGumSingle(videoOnly)) return reason("single-kind evidence malformed");
  if (!["native", "synthetic-pagehide", "none"].includes(lifecycle)
      || (lifecycle === "none") !== (gumResult !== "aborted")) return reason("lifecycle inconsistent with result");
  if (typeof secureContext !== "boolean") return reason("secureContext not boolean");
  const combined = attempts.filter((attempt) => attempt.constraints.audio && attempt.constraints.video);
  const last = combined[combined.length - 1] || null;
  if (gumResult === "unsupported") {
    if (attempts.length !== 0 || audioOnly !== null || videoOnly !== null) return reason("unsupported API cannot record attempts");
  } else {
    if (combined.length < 1 || combined.length > 2 || attempts[0] !== combined[0]) return reason("combined attempt bookkeeping");
    if ((gumResult === "granted") !== (last.outcome === "granted")) return reason("granted result must end in a granted attempt");
    if (gumResult === "timeout" && last.outcome !== "timeout") return reason("timeout result must end in a timed-out attempt");
    const needsSingle = gumResult === "no-device" || gumResult === "device-start-failure";
    const bothSingle = audioOnly !== null && videoOnly !== null;
    const anySingle = audioOnly !== null || videoOnly !== null;
    if (needsSingle ? !bothSingle : anySingle) return reason("single-kind requests must follow no-device/device-start-failure");
    if (attempts.length !== combined.length + (needsSingle ? 2 : 0)) return reason("attempt count");
  }
  const error = last && last.outcome !== "granted" ? { name: last.name, message: last.message } : null;
  const main = byPath.get("permissioned.getUserMedia");
  if (gumResult === "granted" ? main !== undefined : main === undefined) return reason("main getUserMedia record presence");
  if (main && ["device-start-failure", "aborted", "not-supported", "error"].includes(gumResult)
      && (main.status !== "error" || main.error?.name !== last?.name)) return reason("main record must carry the original error");
  return { valid: true, reason: null, attempts, audioOnly, videoOnly, lifecycle, error };
}
...
function validatePermissionedContract(context, payload, rows, byPath) {
  ...
  const trackKinds = [];
  let tracksValid = !tracksMalformed && sameList(trackIndices, trackIndices.map((_, index) => index));
  ...
  // Typed getUserMedia failure model (Codex v4.5). The classification lives in
  // `permissioned.getUserMedia.result`; the typed detail rows keep the original
  // error name/message, constraints, attempt index, duration, visibility and
  // lifecycle state so a failure can be reviewed without weakening READY.
  const gumResult = value("permissioned.getUserMedia.result");
  const gumDetail = validateGetUserMediaEvidence(gumResult, value, byPath);
  if (!gumDetail.valid) out.push(issue("permissioned-getusermedia-evidence-invalid", { context, detail: gumDetail.reason }));

  // Present/absent media kinds come from enumerateDevices BEFORE the prompt.
  const kindByDevice = { audioinput: "audio", videoinput: "video" };
  const presentKinds = [];
  for (const row of rows) {
    const match = /^permissioned\.devices\.before\[(\d+)\]\.kind$/.exec(row?.path || "");
    if (!match || row.status !== "ok") continue;
    const kind = kindByDevice[decodedRecord(row)];
    if (kind && !presentKinds.includes(kind)) presentKinds.push(kind);
  }
  const absentKinds = ["audio", "video"].filter((kind) => !presentKinds.includes(kind));

  // `no-device` is READY-acceptable ONLY when the concrete missing kind is
  // confirmed: every ABSENT kind failed its single-kind request with
  // NotFoundError, and every PRESENT kind was granted with a recorded track.
  let noDeviceConfirmed = false;
  if (gumResult === "no-device" && gumDetail.valid) {
    const single = { audio: gumDetail.audioOnly, video: gumDetail.videoOnly };
    const notFound = ["audio", "video"].filter((kind) => single[kind]
      && single[kind].outcome === "rejected" && single[kind].name === "NotFoundError");
    const grantedKinds = ["audio", "video"].filter((kind) => single[kind]
      && single[kind].outcome === "granted" && single[kind].trackKinds.includes(kind) && trackKinds.includes(kind));
    const combinedNotFound = gumDetail.attempts.some((attempt) => attempt.constraints.audio && attempt.constraints.video
      && attempt.outcome === "rejected" && attempt.name === "NotFoundError");
    noDeviceConfirmed = absentKinds.length > 0 && combinedNotFound
      && sameStringSet(notFound, absentKinds) && sameStringSet(grantedKinds, presentKinds);
    if (!noDeviceConfirmed) {
      out.push(issue("permissioned-no-device-unconfirmed", { context, presentKinds, absentKinds }));
    }
  }
  const requiredTrackKinds = gumResult === "no-device" ? presentKinds : ["audio", "video"];
  if (!sameStringSet(trackKinds, requiredTrackKinds)) tracksValid = false;
  if (!tracksValid) out.push(issue("permissioned-track-evidence-invalid", { context, requiredTrackKinds }));

  if (gumResult === "device-start-failure") {
    out.push(issue("permissioned-getusermedia-device-start-failure", { context, error: gumDetail.error }));
  } else if (gumResult === "aborted") {
    out.push(issue("permissioned-getusermedia-aborted", { context, lifecycle: gumDetail.lifecycle, error: gumDetail.error }));
  } else if (gumResult === "not-supported") {
    out.push(issue("permissioned-getusermedia-not-supported", { context, error: gumDetail.error }));
  } else if (gumResult === "error" || gumResult === "denied" || gumResult === "timeout") {
    out.push(issue("permissioned-getusermedia-failed", { context, result: gumResult, error: gumDetail.error }));
  }
  const gumAcceptable = gumResult === "granted" || (gumResult === "no-device" && noDeviceConfirmed);
  ...
  const expectedOutcomes = {
    granted: gumResult === "granted" ? ["getUserMedia", "geolocation"] : ["geolocation"],
    denied: [], prompt: [],
    unsupported: gumResult === "no-device" ? ["getUserMedia"] : [],
  };
  ...
  const manifestValid = manifest && manifest.phase === "permissioned" && manifest.complete === true
    ...
    && gumAcceptable && manifest.steps.getUserMedia === gumResult && manifest.steps.geolocation === "granted"
...
export function validateSideCapture(input = {}) {
  ...
  // A complete digest is not a complete capture unless the measurement was
  // sealed by the server and its content-addressed payload is present in the
  // durable store inventory. This gate is independent from schema CURRENT.
  let blobPersistenceOk = true;
  const seenBlobIssues = new Set();
  for (const row of records) {
    if (!row || typeof row !== "object" || !row.measurements || typeof row.measurements !== "object") continue;
    const embeddedInventory = Object.prototype.hasOwnProperty.call(row.measurements, "_blobs")
      ? row.measurements._blobs
      : (input.storedBlobInventory ?? input.storedBlobLocators);
    const audit = auditStoredBlobReferences(row.measurements, embeddedInventory);
    if (!audit.ok) blobPersistenceOk = false;
    for (const blobIssue of audit.issues) {
      const key = `${blobIssue.code}\u0000${row.context || ""}\u0000${blobIssue.path || ""}\u0000${blobIssue.locator || ""}`;
      if (seenBlobIssues.has(key)) continue;
      seenBlobIssues.add(key);
      issues.push(issue(blobIssue.code, {
        context: row.context,
        ...(blobIssue.path ? { path: blobIssue.path } : {}),
        ...(blobIssue.locator !== undefined ? { locator: blobIssue.locator } : {}),
      }));
    }
  }
  ...
  // Did the run finish and persist? Independent of the strict verdict: every
  // server-expected context has a record, the browser's run-manifest reached a
  // terminal state for each of them, and nothing was unreadable. A capture can
  // be complete AND NOT_READY (e.g. a typed getUserMedia failure).
  const terminalRealm = (realm) => realm && typeof realm === "object"
    && ["finished", "error", "timeout", "unsupported"].includes(realm.status);
  const terminalManifest = manifests.length === 1 ? manifests[0] : null;
  const terminalPayload = terminalManifest?.measurements;
  const manifestTerminal = terminalPayload?.realms && typeof terminalPayload.realms === "object"
    && sameIdentity(terminalManifest.identity, expectedIdentity)
    && sameIdentity(terminalPayload.identity, expectedIdentity)
    && sameList(terminalPayload.expected, canonical)
    && sameList(Object.keys(terminalPayload.realms), canonical)
    && canonical.every((context) => terminalRealm(terminalPayload.realms[context]));
  const structurallyPersisted = canonical.length > 0 && canonical.every((context) => {
    const list = grouped.get(context);
    if (!list || list.length !== 1) return false;
    const row = list[0], payload = row.measurements;
    if (!sameIdentity(row.identity, expectedIdentity)) return false;
    if (context === "network") return classifyNetworkSchema(payload) === "CURRENT";
    if (payload?._context !== context) return false;
    if (classifySessionSchema(payload) !== "CURRENT") return false;
    return context !== "permissioned" || payload._phaseManifest?.complete === true;
  });
  const captureComplete = structurallyPersisted && manifestTerminal === true && blobPersistenceOk
    && (input.unreadableLines ?? 0) === 0;
...
export function validatePairedCapture({ plain, anti } = {}) {
  ...
  const captureComplete = p.captureComplete && a.captureComplete;
  return { ready, overall: ready ? "READY" : "NOT_READY", captureComplete, plain: p, anti: a, issues };
}
