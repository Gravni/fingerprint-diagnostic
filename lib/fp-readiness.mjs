...
export function validateSideCapture(input = {}) {
  ...
  const manifestTerminal = manifests.length === 1
    && manifests[0].measurements?.realms && typeof manifests[0].measurements.realms === "object"
    && canonical.every((context) => terminalRealm(manifests[0].measurements.realms[context]));
  const captureComplete = canonical.length > 0 && canonical.every((context) => grouped.has(context))
    && manifestTerminal && (input.unreadableLines ?? 0) === 0;
